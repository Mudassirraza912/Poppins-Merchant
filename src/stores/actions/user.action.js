...
export const userLogin = (user) => {
  return async (dispatch) => {
    try {
      ...
      let { data } = await postApi(`${base_url}/merchants/login_merchant`, user)
...
export const forgetPass = (obj) => {
  return async (dispatch) => {
    try {
      dispatch({type: 'FETCHING'})
      let { data } = await postApi(`${base_url}/merchants/forgot_password`, obj)
      const {payload} = data
      if (data.code == 200) {
...
export const changePass = (obj, authToken) => {
  return async (dispatch) => {
    let {data} = await getApi(`${base_url}/merchants/validate_reset_pw_token/`, authToken , authToken)
    console.log("isValid validate_reset_pw_token", data,)
    
    if(data.code == 200) {
      try {
        dispatch({type: 'FETCHING'})
        let { data } = await postApi(`${base_url}/merchants/change_password`, obj, authToken)
...
export const updateProfileImage = (imageUrl) => {
  return async function(dispatch, getState) {
    ...
    dispatch({type: "FETCHING"})
    try {
      let { data } = await postApi(`${base_url}merchants/update_image_url/${userId}`, imageUrl, authToken)
      console.log("data updateProfileImage", data)
      if (data.code == 200) {
        // dispatch({ type: "UPDATED_PROFILE_SUCCESS", payload: data })
        return Promise.resolve({ status: true })
      } else {
